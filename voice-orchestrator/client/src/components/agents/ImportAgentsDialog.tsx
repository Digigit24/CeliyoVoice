...
function AgentList({
  ...
  onImport: (agent: RemoteAgent) => void;
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1 flex items-center gap-2">
                {agent.callType && (
                  <span className="text-xs text-muted-foreground capitalize">{agent.callType}</span>
                )}
            ...
            <Button
              ...
              disabled={isImporting || importMutationPending}
            >
              {isImporting ? (
...
function ProviderError({ message, provider, onRetry }: { message: string; provider: string; onRetry: () => void }) {
  const isCredsMissing = message.includes('credentials') || message.includes('API key') || message.includes('CREDENTIALS_MISSING');
  return (
      ...
      <div className="flex items-start gap-2">
        ...
        <div>
          <p className="text-sm font-medium text-destructive">
            {isCredsMissing ? `${provider} API key not configured` : 'Failed to load agents'}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            {isCredsMissing
              ? `Add your ${provider} API key in Settings → Providers to import agents.`
              : message}
          </p>
          {!isCredsMissing && (
  ...
  return (
    <div className="space-y-3">
      ...
      <AgentList
        ...
        importMutationPending={importMutation.isPending}
      />
    </div>
  );
}

function BolnaTab({ isActive }: { isActive: boolean }) {
  const { data: agents, isLoading, error, refetch } = useRemoteAgents('bolna', isActive);
  const importMutation = useImportBolnaAgent();
  const importAllMutation = useImportAllBolnaAgents();
  const [importingId, setImportingId] = useState<string | null>(null);
  ...
  if (!agents?.length) {
    return (
      <div className="flex h-40 items-center justify-center">
        <p className="text-sm text-muted-foreground">No agents found on Bolna.</p>
      </div>
    );
  }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <div className="flex gap-1 rounded-lg bg-muted p-1">
          {(['omnidim', 'bolna'] as TabId[]).map((tab) => (
            <button
              key={tab}
        ...
        <div className="min-h-[12rem]">
          {activeTab === 'omnidim'
            ? <OmnidimTab isActive={open && activeTab === 'omnidim'} />
            : <BolnaTab isActive={open && activeTab === 'bolna'} />
          }
        </div>
